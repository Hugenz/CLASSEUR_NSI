...
##    print(snake)
    ...
    if (x,y) == (position_bombe_x,position_bombe_y):        # si la position du snake == a position bombe
        if len(snake) >2:
            dernier_element =p1x,p1y= snake[-1][0],snake[-1][1]     # coordonnee du dernier element
            avdernier_element =p2x,p2y= snake[-2][0],snake[-2][1]       # coordonnee de l'avant dernier element
            delay = delay - 20      # on agment la vitesse
            snake = snake[:-2]      # on supprime 2 element
            pygame.draw.rect(ecran, coul4, (dernier_element[0]*c, dernier_element[1]*c, c, c)) # on recolorie le dernier element pour ne pas voir les bout restant
            pygame.draw.rect(ecran, coul4, (avdernier_element[0]*c, avdernier_element[1]*c, c, c))
        else :
            continuer = False
